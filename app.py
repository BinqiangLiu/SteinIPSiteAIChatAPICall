...
st.set_page_config(page_title="SteinIP Website AI Chat Assistant", layout="wide")
st.subheader("Welcome to SteinIP Website AI Chat Assistant.")
st.write("Important notice: This SteinIP Website AI Chat Assistant is offered ONLY for the purpose of assisting users to better interact with SteinIP webiste contents and by no means for any other use. Any user should never interact with the AI Assistant in any way that is against any related promulgated regulations. The user is the only entity responsible for interactions taken between the user and the AI Chat Assistant.")
...
def aichat(query):
    ...
    url = 'https://binqiangliu-steinipsiteaichatapi.hf.space/aisitechat'
    headers = {
...
if st.button('Get AI Response'):
    with st.spinner("AI Thinking...Please wait a while to Cheers!"):    
        if user_query !="" and not user_query.strip().isspace() and not user_query == "" and not user_query.strip() == "" and not user_query.isspace():
            response = aichat(user_query)
            st.write("SteinIP AI Response:")
            st.write(response)
